...
namespace ControlExec.Controls
{
    internal class ControlGame : IControl<GameModel>
    {
        ...
        public ControlGame(string connectionBaseStr, int maxNameSize, int validYear)
        {
            MaxNameSize = maxNameSize;
            _repository = new GameRepository(connectionBaseStr);
        ...
        public ExeResult Create(GameModel model)
        {
            ...
            var game = ControlTools.MapTo<GameModel, Game>(model);
        ...
        public IEnumerable<GameModel> GetAll()
        {
            ...
            ParallelLoopResult res = Parallel.ForEach(result, item =>
                   {
                       gameModel.Add(ControlTools.MapTo<Game, GameModel>(item));
                   });
        ...
        {
            model = ControlTools.MapTo<Game, GameModel>(_repository.GetById(id));
            if (model == null)
            {
        ...
        {
            model = ControlTools.MapTo<Game, GameModel>(_repository.GetByName(name));
        ...
        {
            List<GameModel> gameModel = new List<GameModel>();
            var result = _repository.GetGameByGenre(genreId);
            ParallelLoopResult res = Parallel.ForEach(result, item =>
            {
                gameModel.Add(ControlTools.MapTo<Game, GameModel>(item));
            });
        ...
        {
            List<GameModel> gameModel = new List<GameModel>();
            var result = _repository.GetGameByPublisher(publisherId);
            ParallelLoopResult res = Parallel.ForEach(result, item =>
            {
                gameModel.Add(ControlTools.MapTo<Game, GameModel>(item));
            });
        ...
        {
            Game game = ControlTools.MapTo<GameModel, Game>(model);
            return ControlTools.MapTo<Game, GameModel>(_repository.GetByNameYearPublishing(game));
        }
